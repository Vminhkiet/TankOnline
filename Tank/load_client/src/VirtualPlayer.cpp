...
VirtualPlayer::VirtualPlayer(uint32_t id,
                              ...
                              Metrics* metrics,
                              uint32_t matchId)
    : _id(id), _server(serverAddr), _metrics(metrics), _matchId(matchId)
{
...
{
    int n = PacketBuilder::buildLogin(_buf, sizeof(_buf), _seq, _matchId);
    if (n > 0) {
...
void VirtualPlayer::tick(float shootChance)
{
    ...
    int n = PacketBuilder::buildMove(_buf, sizeof(_buf), _seq, _tick, mx, mz, _matchId);
    if (n > 0) {
        int sent = _sock.sendTo(_server, _buf, n);
        if (sent > 0) {
            _sendTimes[_seq] = Clock::now();
    ...
    if (static_cast<float>(rand()) / RAND_MAX < shootChance) {
        int ns = PacketBuilder::buildShoot(_buf, sizeof(_buf), _seq, _tick, _matchId);
        if (ns > 0) {
